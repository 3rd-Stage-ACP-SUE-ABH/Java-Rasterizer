...
public class Rasterizer { // responsible for rasterizing triangles
    ...
    // this function rasterizes triangles by setting pixels in a pixel buffer. It
    // expects raw 3D triangle data in NDC range.
    {
        ...
        Vec3f[] screenSpaceCoords = viewportTransform(
                new Vec3f[] { triangleCoords[0], triangleCoords[1], triangleCoords[2] });
        ...
        // test every pixel in the bounding box and render those which are inside the
        // triangle.
        Vec3f P;
        ...
        for (traversalX = (int) minX; traversalX < maxX; traversalX++) {
            for (traversalY = (int) minY; traversalY < maxY; traversalY++) {
                ...
                Vec3f test = VecOperator.getBarycentricCoords(screenSpaceCoords[0], screenSpaceCoords[1],
                        screenSpaceCoords[2], P);
                // first test: is this pixel inside the triangle?
                if (test.x() < 0 || test.y() < 0 || test.z() < 0)
                    continue;
                P.setDepth(VecOperator.interpolate(
                        new Vec3f(triangleCoords[0].z(), triangleCoords[1].z(), triangleCoords[2].z()), test));
                ...
                if (fragmentColor == null)
                    continue;
                pixelBuffer[P.x().intValue() + viewportWidth * (viewportHeight - P.y().intValue() - 1)] = fragmentColor
                        .getRGB();
            }
        }
    }
