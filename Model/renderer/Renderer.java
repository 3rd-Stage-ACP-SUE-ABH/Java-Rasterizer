...
public class Renderer {
    ...
    Model modelObject;
    // model data. stored such that index 0 corresponds to the 3 3D coordinates
    // specified by face 0.
    // TODO structure : accommodate loading of multiple models
    // TODO error handling : null handling
    // TODO structure : implement a way to handle objects that have no texture
    // coords, or false texture coords. Also implement a texture loading function
    Vec3f[][] vertexCoords;
    Vec3f[][] textureCoords;
    Vec3f[][] normalCoords;
    ...
    // TODO structure : probably better to use an entire array of lights. Improve
    ...
    public void loadModelData(Model modelObject) {
        ...
        for (int i = 0; i < modelObject.nFaces(); i++) {
            for (int j = 0; j < 3; j++) { // iterate through every face 3 times to get every index
                                          // indexV is a single index, representing one Vec3f object
                int indexV = modelObject.getVertexIndices(i)[j];
                vertexCoords[i][j] = VecOperator.mapToNDC(new Vec3f(modelObject.getVertexCoords(indexV)),
                        modelObject.maxCoord);
                ...
                if (modelObject.nNormals() != 0) { // TODO bug fixing : transform normals in their own way
    ...
    public void renderModel() {
        ...
        // TODO error handling : this function assumes nFaces() always matches coords
        // size
        if (buttonFlag || !modelLoaded)
            return;
        System.out.println("FUNCTION CALL : renderer.rendererModel");
        for (int i = 0; i < modelObject.nFaces(); i++) {
            // future tip, don't change the coordinates values, only copy them, especially
            // if you're updating every frame :)
            // deliver data
    ...
    public BufferedImage getPixelBuffer() {
        return pixelBuffer;
    }
    ...
    public Color[] readTexture(String filePath) throws IOException {
        ...
        for (int i = textureReader.getHeight() - 1; i >= 0; i--) {
            for (int j = 0; j < textureReader.getWidth(); j++) {
                textureData[(textureReader.getHeight() - i - 1) * textureReader.getWidth() + j] = new Color(
                        textureReader.getRGB(j, i));
            }
        }
        return textureData;
    }
