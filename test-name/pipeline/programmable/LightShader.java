...
public class LightShader {
    ...
    private static Color shadeDiffuseLight(Light diffuse) {
        // returns diffuse color scaled by intensity according to angle with respect to
        // normal
    ...
    public static Color shadeChunky(Vec3f surfaceNormal, int numberOfChunks) {
        ...
        for (int i = 0; i < diffuseLights.size(); i++) {
            float intensity = Math.max(
                    dot(surfaceNormal.getNormalized(), diffuseLights.get(i).direction.getNormalized().neg()), 0.0f);
