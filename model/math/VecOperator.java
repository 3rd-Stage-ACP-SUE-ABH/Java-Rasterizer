...
public final class VecOperator {
    ...
    public static Color mulColor(Color u, Color v) { // returns color equal to scaling u by v, clipping to 255
        return new Color(min(u.getRed() * v.getRed() / 255, 255), min(u.getGreen() * v.getGreen() / 255, 255),
                min(u.getBlue() * v.getBlue() / 255, 255));
    }

    public static Color sumColor(Color u, Color v) { // returns plain sum clipping to 255
        return new Color(min(u.getRed() + v.getRed(), 255), min(u.getGreen() + v.getGreen(), 255),
                min(u.getBlue() + v.getBlue(), 255));
    }

    public static Vec3f minus(Vec3f u, Vec3f v) { // non ideal implementation
    ...
    // matrix
    public static Matrix mul(Matrix l, Matrix r) { // TODO error handling : throw error here
        ...
        for (int i = 0; i < l.nRows(); i++) {
            for (int j = 0; j < r.nColumns(); j++) { // init value with 0
                float sum = 0;
    ...
    public static Matrix yRotationMatrix(float angle) { // this is the model transform
    ...
    public static Matrix lookAt(Vec3f eye, Vec3f center, Vec3f up) { // this view transform is a bit of a mess and needs
                                                                     // some work.
    ...
    // this depth resolution conveniently allows us to view the depth map without
    // doing any mappings
    public static Matrix mapToNDC(float maxCoords) { // maps object data from range [-maxCoords, maxCoords] into
                                                     // normalized device coordinate range [-1, 1]
    ...
    public static Matrix viewportTransform(int horizontalPXOffset, int verticalPXOffset, int w, int h) { // maps NDCs to
                                                                                                         // viewport of
                                                                                                         // specified
                                                                                                         // width and
                                                                                                         // height and
    ...
    public static Vec3f getBarycentricCoords(Vec3f A, Vec3f B, Vec3f C, Vec3f P) { // this function is a bit of a black
                                                                                   // box
    ...
    public static float interpolate(Vec3f data, Vec3f barycentricCoords) { // returns interpolated value for a point P
                                                                           // inside a triangle from its vertex data
                                                                           // using P's barycentric coordinates
    ...
    public static String colorBufferToString(int[] colorBuffer) {
        ...
        for (int i = 0; i < colorBuffer.length; i++) {
            ...
            // concatting strings is extremely slow, since each time we concat the string
            // has to be copied, meaning
            ...
            bufferData.append(tempColor.getRed()).append(" ").append(tempColor.getGreen()).append(" ")
                    .append(tempColor.getBlue()).append("\n");
        }
